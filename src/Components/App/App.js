...
import AddStudentLogin from "../StudentLogin/AddStudentLogin/AddStudentLogin";
import StudentLoginList from "../StudentLogin/StudentLoginList/StudentLoginList";
...
export default class App extends Component {
  ...
  updateAssignment = updatedAssignment => {
    this.setState({
      assignments: this.state.assignments.map(assign =>
        assign.assignment_id !== Number(updatedAssignment.assignment_id)
          ? assign
          : Object.assign({}, assign, updatedAssignment)
      )
    });
  };

  deleteAssignment = assignment_id => {
    const newAssignments = this.state.assignments.filter(
      assignment => assignment.assignment_id !== Number(assignment_id)
    );
    this.setState({
  ...
  deleteAssignmentClasses = class_id => {
    const newAssignments = this.state.assignments.filter(
      assignment => assignment.class_id !== Number(class_id)
    );
    this.setState({
      assignments: newAssignments
    });
  ...
  updateClass = updatedClass => {
    this.setState({
      classes: this.state.classes.map(schoolClass =>
        schoolClass.class_id !== Number(updatedClass.class_id)
          ? schoolClass
          : updatedClass
      )
    });
  ...
  deleteClass = class_id => {
    const newClasses = this.state.classes.filter(
      schoolClass => schoolClass.class_id !== Number(class_id)
    );
    this.setState({
      classes: newClasses
    });
      ...
      <div className="App">
        ...
        <MyClassroomContext.Provider value={contextValue}>
          <Switch>
              ...
              component={routeProps => (
                <CalendarDate
                  assignments={this.state.assignments.filter(
                    assignment =>
                      getUTCDate(assignment.due_date)
                        .toString()
                        .slice(0, 15) ===
                      getUTCDate(routeProps.match.params.date)
                        .toString()
                        .slice(0, 15)
                  )}
                  {...routeProps}
            ...
            <PrivateRoute
              exact
              path="/addAssignment/:selectedDate"
              roles={["teacher"]}
              component={routeProps => (
                <AddAssignment classes={this.state.classes} {...routeProps} />
              )}
            />
            ...
            <PrivateRoute
              ...
              component={routeProps => (
                <UpdateAssignment
                  assignment={this.state.assignments.find(
                    assignment =>
                      assignment.assignment_id ===
                      Number(routeProps.match.params.assignment_id)
                  )}
                  classes={this.state.classes}
                  {...routeProps}
                />
            ...
            <PrivateRoute
              ...
              roles={["teacher"]}
              component={routeProps => (
                <ClassesList classes={this.state.classes} {...routeProps} />
              )}
            />
            ...
            <PrivateRoute
              ...
              path="/addClass"
              roles={["teacher"]}
              component={routeProps => <AddClass {...routeProps} />}
            />

            <PrivateRoute
              exact
              path="/updateClass/:class_id"
              roles={["teacher"]}
              component={routeProps => (
                <UpdateClass
                  schoolClass={this.state.classes.find(
                    schoolClass =>
                      schoolClass.class_id ===
                      Number(routeProps.match.params.class_id)
                  )}
                  {...routeProps}
                />
              )}
            ...
            <PrivateRoute
              exact
              path="/users/:user_id/studentlogin"
              roles={["teacher"]}
              component={routeProps => <AddStudentLogin {...routeProps} />}
            />
